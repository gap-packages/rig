### Construct the list of all homogeneous quandles of order n up to isomorphism
### Written by Leandro Vendramin
LoadPackage("rig");
LogTo("log");
...
for t in [1..N] do
  ...
  for h in center do
    ...
    ### The followin criterium goes back to Yetter and his students
    ...
    r := Rack(m);
    r.inn := inn;
    if not IsIndecomposable(r) then
      continue;
    fi;
...
i := 1;
Print("## size ", n, "\n");
Print("SMALL_H_QUANDLES[", n, "] := rec( total := -1, implemented := ", Size(q), ", size := ", n, ", rack := [ ] );\n");
for x in q do
  Print("SMALL_H_QUANDLES[", n, "].implemented := ", i, ";\n");
  Print("SMALL_H_QUANDLES[", n, "].rack[", i, "] := rec( size := ", n, ", matrix :=\n");
  Print(x.matrix, ");\n");
  i := i+1;
od;
